...
public class LT042_Trapping_Rain_Water {
	...
	public int trap2(int[] height) {
		// O(3*n). 每个A[i]能trapped
		// water的容量，取决于height[i]左右两边的高度（可延展）较小值与height[i]的差值，即volume[i] =
		// [min(left[i], right[i]) - height[i]] * 1
		...
		for (int i = 0; i < height.length; i++) {
			...
			max = Math.max(max, left[i]); // the same as Math.max(max,
											// height[i])
		}
		// right to left
		...
		for (int i = height.length - 1; i >= 0; i--) {
			...
			max = Math.max(max, right[i]); // the same as Math.max(max,
											// height[i])
		}
