...
public class LT030_Substring_With_Concatenation_Of_All_Words {
    public List<Integer> findSubstringSol(String s, String[] words) {
        ...
        for(int i=0;i<=s.length()-n*single;i++){                                //loop starting index
            HashMap<String, Integer> sliding= new HashMap<>(dict);
            for(int j=0;j<n;j++){
                String curWord =  s.substring(i+j*single, i+(j+1)*single);      //search for each word
                if(sliding.containsKey(curWord)){                               //update in the sliding HashMap
                ...
                if(sliding.size()==0) res.add(i);                               //if sliding window contains all word. add index i
            }
        }
        return res;
    }
