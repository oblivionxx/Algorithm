        ...
        for (int i=0;i<len;i++) {
            while (true) {
                ...
                for (int j=i+1;j<len;j++) {                 //loop all strings
                    if (ans[j].equals(ans[i])) set.add(j);  //check all strings with the same abbreviation. store index
                }
                if (set.isEmpty()) break;                   //if not empty, need to recalculate abbreviation
                set.add(i);
                for (int k: set) 
