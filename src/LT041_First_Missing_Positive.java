...
public class LT041_First_Missing_Positive {
	...
	public int firstMissingPositive(int[] nums) {
		/*
		 * We must have nums[i]<=n We only have n numbers, and the answer must
		 * be in [1, n + 1]. 1. If n numbers are 1, 2, 3, ..., n, then the
		 * answer is n + 1. 2. use a number x (x <= 0 || x > n) to replace a
		 * number in 1, 2, 3, ..., n. There must be a number less than n+1.
		 * 不能按照A[i] = i来存，因为题目要求寻找正数，如果A[0]用来存储0的话，会影响数据处理。比如当A = {1}的时候，如果A[0]
		 * = 0的话，根本没地方存1的值了。所以正确的存储方式应该是A[i]= i+1.
		 */
