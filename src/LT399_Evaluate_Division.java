...
Graph
 */
public class LT399_Evaluate_Division {
	...
	for (int i = 0; i < equations.length; i++) {
	    ...
	    valuesPair.get(equation[1]).add(1 / values[i]); // build two map to store node and link relationship
	}

	double[] result = new double[queries.length];
	for (int i = 0; i < queries.length; i++) {
	    String[] query = queries[i];
	    result[i] = dfs(query[0], query[1], pairs, valuesPair, new HashSet<String>(), 1.0); // do dfs
	    if (result[i] == 0.0)
		result[i] = -1.0;
	}
	return result;
