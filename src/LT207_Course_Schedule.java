...
public class LT207_Course_Schedule {
	...
	for (int i = 0; i < prerequisites.length; i++) {
	    posts.get(prerequisites[i][1]).add(prerequisites[i][0]); // store
								     // the
								     // pre-course.
								     // of node
								     // i.
	...
	int[] preNums = new int[numCourses]; // for arbitrary node i, sum up
					     // total pre-course.
	for (int i = 0; i < numCourses; i++) {
	...
	for (int i = 0; i < numCourses; i++) {
	    ...
	    // if not find a non-pre course. there's no node with in-degree=0.
	    // which also means there's a cycle in the graph
	    if (j == numCourses)
		return false;

	    preNums[j] = -1; // set it as start

	    // decrease courses that post the course //remove the links==>
	    // decrease the courses
