...
public class LT085_Maximum_Rectangle {
	...
	/*
	 * Open matrix from top to the bottom line by line. Let the maximal
	 * rectangle area at row i and column j be computed by [right(i,j) -
	 * left(i,j)]*height(i,j). left(i,j) = max(left(i-1,j), curleft), curleft
	 * can be determined from the current row right(i,j) = min(right(i-1,j),
	 * curright), curright can be determined from the current row height(i,j) =
	 * height(i-1,j) + 1, if matrix[i][j]=='1'; height(i,j) = 0, if
	 * matrix[i][j]=='0';
	 */
		...
		for (int i = 0; i < m; i++) {
			...
			for (int j = 0; j < n; j++) {
				if (matrix[i][j] == '1')
					...
					left[j] = 0; // can ensure left[j] is continuous 1s on the
									// left.
					cur_left = j + 1;
				}
			}
