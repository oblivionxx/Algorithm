...
public class LT041_First_Missing_Positive {
	...
	public int firstMissingPositive(int[] nums) {
		...
		//nums[i] should be in position nums[i]-1. swap nums[i], nums[nums[i]-1]. 
	    //swap nums. so that the array is starting from 1.2.3...
	    //3.4.-1.1->-1.4.3.1.->-1.1.3,4 ->1.-1.3.4->-1 is not in the correct position
		...
		for(int i=0;i<nums.length;i++){
            if(i!=nums[i]-1){	
                if(nums[i]<=0 || nums[i]>nums.length || nums[i]==nums[nums[i]-1]) continue;
                    ...
                    nums[i] = nums[temp-1];  
                    nums[temp-1] = temp;
                    i--;						//recheck the cur point. swap until it's correct or out of range
                }
            }
        }
