...
public class LT079_Word_Search {
	...
	public boolean exist(char[][] board, String word) {
        ...
        for(int i=0;i<m;i++){
            for(int j=0;j<n;j++){
                if(helper(board,word,i,j, visited)) return true;
            }
        }
    ...
    public boolean helper(char[][] board, String word, int i, int j, boolean[][] visited){
        if(!visited[i][j]){
            ...
            if(board[i][j]==word.charAt(0)){
                visited[i][j] = true;
                if(i+1<m && helper(board, word.substring(1), i+1, j, visited)) return true;
                if(i-1>=0 && helper(board, word.substring(1), i-1, j, visited)) return true;
                if(j+1<n && helper(board, word.substring(1), i, j+1, visited)) return true;
                if(j-1>=0 && helper(board, word.substring(1), i, j-1, visited)) return true;
                visited[i][j] = false;
            }
        }
        return false;
