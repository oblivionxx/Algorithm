...
public class LT010_Regular_Expression_Matching {
	public boolean isMatch(String s, String p) {
		// dp O(n^2)
		/*
		 * boolean dp[i][j]: if s[0..i-1] matches p[0..j-1] if p[j-1] != '*'
		 * dp[i][j] = (dp[i-1][j-1] && (s[i-1] == p[j-1]||p[j-1]=='.')) else
		 * p[j-1] = '*', p[j-2]=x then dp[i][j] is true iff any of the following
		 * is true 1) "x*" repeats 0 time and matches empty: dp[i][j -2] 2) "x*"
		 * repeats >= 1 times and matches s->"x*x": dp[i-1][j] && s[i-1] == x.
		 * (dp[i-1][j] already satisfy the smth like s=xx, p=x*. then new s =
		 * xxx. still match p)
		 */
