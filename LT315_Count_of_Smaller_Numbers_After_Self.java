    ...
    public int insertNode(TreeNode root, int val) {
		...
		while(true) {
			if(val <= root.val) {                               //[1,3]->starting from root 3. 
				root.count++;                                   //root.count save the number of asc nums on the right. count updated. but not stored in res list
				if(root.left == null) {
				...
				thisCount += root.count;                        //thisCount is used for count for new Number. default =1
				if(root.right == null) {
